...
FIG3       := $(ASSETS)/figure3_correlational.pdf
...
TAB4       := $(TABLES)/table4_causality.txt

FIGURE_FILES := $(FIG1) $(FIG2) $(FIG3) $(FIG_CONF)
TABLE_FILES  := $(TAB1) $(TAB2) $(TAB3) $(TAB4) $(TAB5_DSPEC) $(TAB5_SCORE) $(TAB789) $(TAB10)
...
.PHONY: help all figures tables clean \
        figure1 figure2 figure3 figure-confound \
        table1 table2 table3 table4 table5-dspec table5-score table789 table10

help:
	@echo "Targets:"
	@echo "  make all            # every reproducible figure + table"
	@echo "  make figures        # $(notdir $(FIGURE_FILES))"
	@echo "  make tables         # $(notdir $(TABLE_FILES))"
	@echo ""
	@echo "  Figures:  figure1  figure2  figure3  figure-confound"
	@echo "  Tables:   table1  table2  table3  table4  table5-dspec  table5-score  table789  table10"
	@echo ""
...
# 𝒟 (dspec_cosine) vs HaluEval-Summarization over the 39 ND-LoRA_P4 draws in evals-N128.
$(FIG3): $(ADHOC)/figure3_correlational.py $(ADHOC)/table5_dspec_ablations.py | $(ASSETS)
	$(RUN) $(ADHOC)/figure3_correlational.py
...
# Table 4: causal dose-response from the corruption suite (scripts/eval_neurodiversity.py --corruption-mode stream).
$(TAB4): $(ADHOC)/table4_causality.py utils/paired_stats.py $(DATADEPS) | $(TABLES)
	$(RUN) $(ADHOC)/table4_causality.py > $@ 2>&1
...
clean:
	rm -f $(FIGURE_FILES) $(ASSETS)/figure1_optimal_diversity.png \
	      ...
	      $(ASSETS)/figure3_correlational.png
