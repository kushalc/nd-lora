...
@dataclass
class BaseModelOutputWithStreamHiddenStates(BaseModelOutputWithPast):
    """
    Base class for model's outputs that may also contain individual stream hidden states.
    ...
    Args:
        ...
        attentions (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            sequence_length)`.
        stream_hidden_states (`torch.FloatTensor`, *optional*, returned when `return_stream_hidden_states=True`):
            Individual stream hidden states of shape `(batch_size, sequence_length, num_streams, hidden_size)`.
    """
    stream_hidden_states: Optional[torch.FloatTensor] = None


@dataclass 
class CausalLMOutputWithStreamLogits(CausalLMOutputWithPast):
    """
    Base class for causal language model (or autoregressive) outputs with individual stream logits.
    ...
    Args:
        ...
        attentions (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
            sequence_length)`.
        stream_logits (`list` of `torch.FloatTensor`, *optional*, returned when `return_stream_logits=True`):
            List of individual stream logits, each of shape `(batch_size, sequence_length, config.vocab_size)`.
    """
    stream_logits: Optional[List[torch.FloatTensor]] = None
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        return_stream_hidden_states: Optional[bool] = False,
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.parscale_n > 1:
            # Capture individual stream hidden states before aggregation
            individual_stream_hidden_states = rearrange(hidden_states, "(n_parscale b) s h -> b s n_parscale h", n_parscale=self.parscale_n)
            if return_stream_hidden_states:
                stream_hidden_states = individual_stream_hidden_states
            ...
            hidden_states = torch.sum(
                individual_stream_hidden_states * attn, 
                dim=2, keepdim=False
        ...
        output = BaseModelOutputWithStreamHiddenStates(
...
class Qwen2ParScaleForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        return_stream_logits: Optional[bool] = False,
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        outputs = self.model(
            ...
            cache_position=cache_position,
            return_stream_hidden_states=return_stream_logits,
            **kwargs,
        )
        ...
        # Compute individual stream logits if requested
        stream_logits = None
        if return_stream_logits and hasattr(outputs, 'stream_hidden_states') and outputs.stream_hidden_states is not None:
            # outputs.stream_hidden_states shape: [batch_size, seq_len, num_streams, hidden_size]
            batch_size, seq_len, num_streams, hidden_size = outputs.stream_hidden_states.shape
            
            # Handle num_logits_to_keep for stream logits
            stream_hidden_to_use = outputs.stream_hidden_states[:, -num_logits_to_keep:, :, :] if num_logits_to_keep > 0 else outputs.stream_hidden_states
            
            # Reshape to apply lm_head: [batch_size * seq_len * num_streams, hidden_size]
            stream_hidden_flat = stream_hidden_to_use.reshape(-1, hidden_size)
            stream_logits_flat = self.lm_head(stream_hidden_flat)
            
            # Reshape back and split by stream: [batch_size, seq_len, num_streams, vocab_size]
            seq_len_to_use = stream_hidden_to_use.shape[1]
            stream_logits_tensor = stream_logits_flat.reshape(batch_size, seq_len_to_use, num_streams, self.config.vocab_size)
            
            # Convert to list of individual stream logits
            stream_logits = [stream_logits_tensor[:, :, i, :] for i in range(num_streams)]
        ...
        return CausalLMOutputWithStreamLogits(
            loss=loss,
            logits=logits,
