...
def compute_paired_ttest(treatment_df: pd.DataFrame, baseline_df: pd.DataFrame,
    ...
    # Merge treatment and baseline on group_cols (plus doc_id when present) to create paired observations
    merge_cols = group_cols + (['doc_id'] if 'doc_id' in treatment_df.columns
                               and 'doc_id' in baseline_df.columns else [])
    merged = treatment_df.merge(baseline_df, on=merge_cols, how='inner', suffixes=('_treatment', '_baseline'))
